...
def absmean(input):
    dtype = input.dtype
    input = input.float()
    s = input.abs().mean().clamp(min=1e-5)
    result = (input / s).round().clamp(-1, 1) * s
    return result.type(dtype)


def quantize_to_int2(weight: torch.Tensor,):
    weight = weight.float()
    step = weight.abs().mean().clamp(min=1e-5)
    q = (weight / step).round().clamp(-1, 1).to(torch.uint8) + 1
    ...
    if remainder != 0:
        padding = 4 - remainder
        q = torch.cat([q, torch.zeros(padding, dtype=torch.uint8, device=q.device)])
    q = q.view(-1, 4)
...
# @torch.compile
def dequantize_from_int2(packed: torch.Tensor, step: float, orig_shape, n_elems: int):
    q0 = (packed & 0x03)
    q1 = ((packed >> 2) & 0x03)
    q2 = ((packed >> 4) & 0x03)
    q3 = ((packed >> 6) & 0x03)
    q_all = torch.stack([q0, q1, q2, q3], dim=1).view(-1)
    q_all = q_all[:n_elems]  # 截去补齐部分
    q_all = q_all.float() - 1
    weight = q_all * step
...
class BitLinear(nn.Linear):
    ...
    def quantize_weights(self):
        if self.weight_bits == 32:
            return
        packed, step, orig_shape, n_elems = quantize_to_int2(self.weight.data)
        self.register_buffer("q_weight", packed)
        self.register_buffer("w_step", torch.tensor(step, dtype=torch.float32))
        self.orig_shape = orig_shape
        self.n_elems = n_elems
        qweight = dequantize_from_int2(
            self.q_weight, 
            self.w_step.item(), 
            self.orig_shape, 
            self.n_elems,
        ).type(torch.bfloat16)
        assert (torch.isclose(qweight, absmean(self.weight)) == False).sum() == 0
        self.weight = None
        self.enable_qlora = True
...
class SiglipAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.vit_weight_bits = getattr(config, "vit_weight_bits", 32)
        self.vit_weight_quant_method = getattr(config, "vit_weight_quant_method", "absmean")
        self.vit_act_bits = getattr(config, "vit_act_bits", 32)
        self.vit_act_quant_method = getattr(config, "vit_act_quant_method", "absmax_per_token")
        print(f"siglip attention, weight bits: {self.vit_weight_bits}, weight quant method: {self.vit_weight_quant_method}, act bits: {self.vit_act_bits}, act quant method: {self.vit_act_quant_method}")

        self.k_proj = BitLinear(self.embed_dim, self.embed_dim, weight_bits=self.vit_weight_bits, input_bits=self.vit_act_bits)
        self.v_proj = BitLinear(self.embed_dim, self.embed_dim, weight_bits=self.vit_weight_bits, input_bits=self.vit_act_bits)
        self.q_proj = BitLinear(self.embed_dim, self.embed_dim, weight_bits=self.vit_weight_bits, input_bits=self.vit_act_bits)
        self.out_proj = BitLinear(self.embed_dim, self.embed_dim, weight_bits=self.vit_weight_bits, input_bits=self.vit_act_bits)
...
class SiglipMLP(nn.Module):
    def __init__(self, config):
        ...
        self.vit_act_bits = getattr(config, "vit_act_bits", 32)
        self.vit_act_quant_method = getattr(config, "vit_act_quant_method", "absmax_per_token")
        print(f"siglip mlp, weight bits: {self.vit_weight_bits}, weight quant method: {self.vit_weight_quant_method}, act bits: {self.vit_act_bits}, act quant method: {self.vit_act_quant_method}")
        self.fc1 = BitLinear(config.hidden_size, config.intermediate_size, weight_bits=self.vit_weight_bits, input_bits=self.vit_act_bits)
        self.fc2 = BitLinear(config.intermediate_size, config.hidden_size, weight_bits=self.vit_weight_bits, input_bits=self.vit_act_bits)
...
class SiglipVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        last_hidden_state = encoder_outputs[0]
        last_hidden_state = self.post_layernorm(last_hidden_state) if self.use_head else None
